	...
	if ($row > 0) {
		...
		if ($password == $re_password && md5($password) == $result['password']) {
			...
			if (strlen($_POST['password1']) <= 5 || strlen($_POST['re_password1']) <= 5) {
				...
				header('Location: http://localhost/project_blk/v.1.0.3/staf/dashboard?ganti_password=true&warning='. urlencode('Password harus lebih dari 5 karakter'));
				...
				if ($_POST['password1'] == $_POST['re_password1']) {
					...
					if ($username_baru != '') {
							...
							if ($runUpdate) {
								...
								header('Location: http://localhost/project_blk/v.1.0.3/staf/dashboard?ganti_password=true&success='. urlencode('Password sudah diubah'));

							}else{

								header('Location: http://localhost/project_blk/v.1.0.3/staf/dashboard?ganti_password=true&err='. urlencode('Password gagal diubah'));
					...
					header('Location: http://localhost/project_blk/v.1.0.3/staf/dashboard?ganti_password=true&err='. urlencode('Password baru tidak sama<br>'));
			...
			header('Location: http://localhost/project_blk/v.1.0.3/staf/dashboard?ganti_password=true&err='. urlencode('Password lama tidak sama<br>'));
